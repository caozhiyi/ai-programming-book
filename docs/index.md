...
你需要有编程经验——不需要是资深工程师，但至少写过真实的项目代码，理解什么是 API、什么是版本控制、什么是代码审查。你正在使用或即将使用 AI 编程工具——Cursor、GitHub Copilot、Windsurf、或者任何基于大模型的编程助手。你想从"能用"进阶到"用好"——不只是让 AI 帮你写代码，而是理解它的能力边界，在正确的场景用正确的方式使用它。

如果你是完全没有编程经验的初学者，这本书可能不太适合作为起点。如果你是机器学习研究者，想深入了解 Transformer 架构的数学细节，这本书也不是你要找的——我们关注的是工程应用层面的原理，而不是论文级别的理论推导。

## 全书结构

全书分为五卷，十七章，沿着一条递进的逻辑主线展开。
...
**卷三：记忆与上下文（第 8-10 章）。** AI 系统怎么"记住"你——记忆体系的设计、上下文窗口的高效利用、以及当模型的训练数据不够时如何注入外部知识（RAG、微调、长上下文）。

**卷四：架构选型的判断力（第 11-12 章）。** 工具箱满了，但知道该用哪个才是真本事——选型决策框架帮你判断什么场景用什么方案，规范驱动编程帮你把 AI 的输出从"碰运气"变成"可预期"。

**卷五：工程化与未来（第 13-17 章）。** 从系统蓝图到安全设计，从非确定性系统的质量保障到组织治理，最终到 AI 编程的能力边界和演进方向。

五卷之间是递进关系：不理解模型原理，就无法理解 Agent 为什么这样设计；不理解 Agent 机制，就无法理解上下文工程为什么重要；不理解上下文工程，就无法做出正确的架构选型；不理解架构选型，就无法建立可靠的工程化实践。每一卷都是下一卷的前提。
...
有一点需要提前说明：这本书不会过时得很快。AI 编程工具的产品形态会不断变化——今天的 Cursor 可能明天就被新产品替代。但底层的原理不会变——Token 化的机制、注意力的物理约束、Agent 的执行逻辑、非确定性系统的工程化挑战——这些是由大模型的架构决定的，不会因为某个产品的更新而失效。这本书教你的是"钓鱼"，而不是"给你一条鱼"。