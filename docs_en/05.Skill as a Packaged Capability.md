...
Finally, one boundary worth stating cleanly: **the difference between a Skill and a Tool is not "who chooses," it is "how the chosen thing acts afterward."** As 5.3 made explicit, in mainstream implementations a Skill is loaded by an LLM-driven probabilistic selection over a list of summaries—mechanically the same as how Tool calls are chosen. In both cases the model decides and the Agent executes. The real difference shows up *after* loading. A Tool is discrete: it is called once, returns a value, and ends; its influence on subsequent generation flows only through that returned value. A Skill is continuous: once its body is in context, it sits there as a background constraint, shaping the probability of every token thereafter. One is *"do a thing and leave,"* the other is *"sit in the context and keep influencing."* Keep that boundary in mind and you will not reach for a Skill in a situation that calls for a Tool, or for a Tool in a situation that calls for a Skill.
