...
## 3.2 ReAct: A Loop of Reasoning and Action
...
If you have spent enough time with AI coding systems, you have probably seen this pattern. The model did not fail to inspect its own work. It did inspect it. The problem was that the inspection was grounded in a mistaken frame. The result can look disciplined, thorough, and still be wrong.
...
That pattern shows up across practical AI coding tools. The specific product varies, but the logic stays the same: let the Agent do the browsing, parsing, comparing, and planning it is good at; stop it when irreversible side effects are about to happen.
