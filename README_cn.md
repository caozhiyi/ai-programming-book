...
<p>
  ...
  <a href="#-章节目录"><img src="https://img.shields.io/badge/📚_章节-4卷_15章-4CAF50?style=for-the-badge" alt="章节"></a>
...
全书分为 **4 卷 15 章**，沿着一条**严格递进**的逻辑主线展开。每一卷都是下一卷的前提。
...
- **第 10 章** [知识注入：从 RAG 到代码库原生检索](docs/10.知识注入：从%20RAG%20到代码库原生检索.md) ⭐

### 📕 卷四：工程化与组织化

> 零件装成系统之后，问题开始往真实代码库、真实生产、真实团队、真实组织里渗透。

- **第 11 章** [规范驱动编程：当需求变成工程产物](docs/11.规范驱动编程：从提示词到OpenSpec.md) —— 当模型成为开发流程的一部分，规范要直接进 Git、进 CI
- **第 12 章** [安全与对齐：AI 系统的信任边界](docs/12.安全与对齐：AI系统的信任边界.md) —— Agent 能动手做事之后，信任边界必须从架构第一天就设计进去
- **第 13 章** [判定权重建：AI 写的代码该怎么审](docs/13.非确定性系统的工程化挑战.md) —— 测试、回归、调试这套老工具箱在面对概率系统时，哪些要重做
- **第 14 章** [当 AI 进入团队：协同重排](docs/14.当AI进入团队：协同重排.md) —— 个人闭环和团队闭环不再是同一件事，协同方式被重新挤压
- **第 15 章** [当 AI 进入组织：从工具到能力底座](docs/15.当AI进入组织：从工具到能力底座.md) ⭐ —— 几百个 Skill 谁来维护，中层管理的合理性有没有发生变化
...
### 2. **配套 20+ 张原创结构图**
关键机制（Token 化对比、ReAct 循环、MCP 架构、Skill 动态加载、记忆流水线、信任边界分层……）全部配有 SVG 结构图，**看图就能理解一半**。
...
- 想理解 **AI 在大型代码库里怎么找到相关代码** → 直接读第 10 章
- 想搞清楚 **AI 进入团队后协作怎么变** → 直接读第 14 章
- 想搭 **组织级的 AI 能力底座** → 直接读第 15 章
